...
**Problème** : `activeFieldName: string | null` ne supporte qu'UNE carte ouverte. Il faut que parent + enfants soient tous ouverts simultanément. Lorsqu'on ferme le parent, les enfants doivent aussi être fermés.
...
- Détecter si le schema contient au moins un champ avec `branchingEnabled === true`
- Si oui, passer `readOnly={false}` pour permettre à l'admin de voir l'embranchement en action
...
- Passer `onChange={setPreviewInteractiveValues}` au DynamicForm
- Reset `previewInteractiveValues` quand le schema change
- Si aucun embranchement dans le schema, garder `readOnly={true}` comme avant
