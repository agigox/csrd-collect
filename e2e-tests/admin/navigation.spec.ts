...
import { mockFormTemplates, mockCategoryCodes } from "../helpers/mock-data";
...
test.describe("Navigation admin", () => {
  ...
  test("la sidebar contient les éléments de menu admin", async ({ page }) => {
    await page.goto("/admin");
  ...
  test("cliquer sur Admin. d'équipe navigue vers la bonne route", async ({
    ...
    await expect(page).toHaveURL(/\/admin\/teams-admin/);
  });
  ...
  test("cliquer sur Déclarations navigue vers /declarations", async ({
    ...
    await page
      .locator("nav")
      .getByRole("link", { name: "Déclarations" })
      .click();
